    ...
    for inputs, labels in dataloader:
        ...
        with torch.set_grad_enabled(False):
            preds = ensemble_prediction_pooling(
                list_models, inputs, pool_op=np.mean
            )
...
def ensemble_prediction_pooling(list_models, inputs, pool_op=np.mean):
    ...
    outputs = []
    for model in list_models:
        outputs.append(model(inputs).cpu().items())
    # Let's make average pool
    # outputs : B x C x 1
    output = np.concat(outputs, axis=1)
    output = pool_op(output, axis=1)
    return output


def ensemble_pooling(outputs, pool_op=np.mean):
    output = np.concat(outputs, axis=1)
    output = pool_op(output, aixs=1)
    return output
...
if __name__ == '__main__':
    ...
    list_files_2017_eol = pickle.load(open(
        '../list_files_2017_eol_commons_unique.pkl', 'rb'))
    list_files_2017_web = pickle.load(open(
        '../list_files_2017_web_commons_unique.pkl', 'rb'))
    ...
    image_dataset = PlantCLEFDataSet(trainset[0], list_files_all,
                                      data_transform)
