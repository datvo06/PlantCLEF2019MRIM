...
class PlantCLEFDataSetWeightOversamp(object):
    def __init__(self, class_id_map, class_id_list_files,
        ...
        for each_class_id in range(len(self.class_id_list_files)):
            self.filepath_merged += self.class_id_list_files[each_class_id]
...
def get_unique_samples_from_dataset1(trainset1, trainset2):
    inv_class_1 = dict([(pair[1], pair[0]) for pair in trainset1[0].items()])
    inv_class_2 = dict([(pair[1], pair[0]) for pair in trainset2[0].items()])
    ...
    for each_class in commons_classes:
        list_files1 = trainset1[1][inv_class_1[each_class]]
        list_files2 = trainset2[1][inv_class_2[each_class]]
        ...
        for fsize_idx in file_sizes2_idx:
            while (curr_idx < len(file_sizes1) and
                ...
                list_files1_out.append(list_files1[file_sizes1_idx[curr_idx]])
                curr_idx += 1
                while (curr_idx < len(file_sizes1) and
                        file_sizes1[file_sizes1_idx[curr_idx]] ==
                        file_sizes2[fsize_idx]):
